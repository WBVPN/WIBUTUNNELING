...
create_account() {
    ...
    pesan+="ISP            : ${ISP}\n"
    pesan+="Domain         : ${domain}\n"
    pesan+="Limit IP       : ${limit_ip}\n"
    pesan+="Limit Kuota    : ${limit_bw}\n"
    pesan+="Port TLS       : 443\n"
    
    if [[ "$proto" != "TROJAN" ]]; then
        pesan+="Port none TLS  : 80\n"
    fi
    
    if [[ "$proto" == "VMESS" ]]; then
        pesan+="id             : ${uuid}\n"
        pesan+="Network        : ws,grpc\n"
        pesan+="Path ws        : /vmess\n"
        pesan+="serviceName    : vmess\n"
    elif [[ "$proto" == "VLESS" ]]; then
        pesan+="id             : ${uuid}\n"
        pesan+="Encryption     : none\n"
        pesan+="Network        : ws,grpc\n"
        pesan+="Path ws        : /vless\n"
        pesan+="serviceName    : vless\n"
    elif [[ "$proto" == "TROJAN" ]]; then
        pesan+="Password       : ${uuid}\n"
        pesan+="Network        : ws,grpc\n"
        pesan+="Path ws        : /trojan\n"
        pesan+="serviceName    : trojan\n"
    fi

    pesan+="Expired On     : ${tampil_exp}\n"
    pesan+="${THICKLINE}\n"
    pesan+="            ${proto} WS TLS\n"
    pesan+="${THICKLINE}\n"
